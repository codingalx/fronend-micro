import {
  ...
  import {  getAllQualification, getEducationOpportunityById, getEmployeeByEmployeId, listAllCountry, listEducationLevels, listOfBudgetYears, updateeducationOpportunity } from "../../../configuration/TrainingApi";
...
import { useIds } from "../../context/IdContext";
  ...
  const UpdateEductionOpportunity = () => {
    ...
    const educationOpportunityId = location?.state?.id
    ...
    const fetchEducationOpportunity = async () => {
        try {
          const response = await getEducationOpportunityById(tenantId,educationOpportunityId);
          const data = response.data; 
          setEducationOpportunity(data);
          console.log("the fetched adrress are",data);
        } catch (error) {
   ...
   const fetchCountries = async () => {
     try {
       ...
       setNotification({ open: true, message: "Failed fetch the Country Name c. Please try again.", severity: "error" });
     }
   };
    ...
    const fetchEmployeeDetails = async (employeeId) => {
      try {
        const response = await getEmployeeByEmployeId(tenantId,employeeId);
    ...
    const fetchQualifications = async () => {
      try {
        const response = await getAllQualification(tenantId);
    ...
    const handleFormSubmit = async (values, { resetForm }) => {
      try {
        await updateeducationOpportunity(tenantId,educationOpportunityId,values);
    ...
    const checkoutSchema = yup.object().shape({
        budgetYearId: yup
            .string()
            .required("Budget year  cannot be null"),
            countryId: yup
            .string()
            .required("country name cannot be null"),
    ...
    return (
      <Box m="20px">
        ...
        />
        <Header subtitle="Update Education Opportunity" />
  
        <Formik
          onSubmit={handleFormSubmit}
          initialValues={educationOpportunity}
  ...
  export default UpdateEductionOpportunity;
