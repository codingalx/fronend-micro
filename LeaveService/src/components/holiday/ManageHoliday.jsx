...
const ManageHoliday = ({refreshKey}) => {
  ...
  const checkPermissions = async () => {
    try {
      const editAccess = await canAccessResource(
        LeaveServiceResourceName.UPDATE_HOLIDAY,
        userRoles
      );
      const deleteAccess = await canAccessResource(
        LeaveServiceResourceName.DELETE_HOLIDAY,
        userRoles
      );
      const getDetails = await canAccessResource(
        LeaveServiceResourceName.GET_HOLIDAY_BY_ID,
        userRoles
      );
 ...
 try {
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => {
        const onEditClick = () => {
          navigate("/editholiday", { state: { id: params.row.id } });
        };
    ...
    },
    {
      ...
      renderCell: (params) => {
        const onEditClick = () => {
          navigate("/detailsholiday", { state: { id: params.row.id } });
        };
  ...
  return (
    <Box m="20px">
      <Header
        subtitle="Managing the holiday list"
      />
      <Box
        m="40px 0 0 0"
        height="75vh"
