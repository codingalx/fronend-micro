...
import {
  ...
  updateGoodReceivingNote,
} from "../../Api/storeMovementAp";
import { useLocation,useNavigate } from "react-router-dom";

const UpdateGoodReceivingNote = () => {
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    try {
      await updateGoodReceivingNote(tenantId,id, values);
      setNotification({
        open: true,
        message: "good receiving notes updated successfully!",
        severity: "success",
      });
      resetForm();
        navigate('/storeMovent/create_goodReceiving_note'); //
     
    } catch (error) {
      console.error("Failed to submit form data:", error);
      setNotification({
        open: true,
        message: "Failed to update good receiving note. Please try again.",
        severity: "error",
      });
    }
  };
   ...
   const [goodReceivingGood, setGoodReceivingGood] = useState({
       type: "",
    inspectionId: "",
    storeId: "",
    receivedDate: "",
    ...
    grn_NO: "",
      ...
      const fetchReceivingNoteById = async () => {
                try {
                    const response = await getGoodReceivingNoteById(tenantId,id);
                    setGoodReceivingGood(response.data);
               
                 } catch (error) {
                  console.error("Error fetching good receiving note:", error.message);
        }
            };
  ...
  return (
    <Box m="20px">
      ...
      <Formik
        ...
        initialValues={goodReceivingGood}
...
export default UpdateGoodReceivingNote;
