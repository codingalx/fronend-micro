...
import {
  getAllItems,
  getAllStores,
  getGatePassInformationById,
  updateGatePassInformation,
} from "../../Api/storeMovementAp";

import { useLocation, useNavigate } from "react-router-dom";

const UpdateGatePassInformation = () => {
  ...
  const [gatepassInformation, setGatepassInformation] = useState({
    movementType: "",
    gatePassNo: "",
    movementTypeNo: "",
    requestedBy: null,
  ...
  const fetchGatePassInfomationById = async () => {
    try {
      const response = await getGatePassInformationById(tenantId, id);
      setGatepassInformation(response.data);
    } catch (error) {
      console.error("Error fetching good gate pass infomation:", error.message);
    }
  };
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    ...
    const adjustedValues = {
      ...values,
      movementTypeNo:
        values.movementType === "TRANSFER" ? null : values.movementTypeNo,
      requestedBy:
        values.movementType !== "TRANSFER" ? null : values.requestedBy,
    };

    try {
      await updateGatePassInformation(tenantId, id,values);
      setNotification({
        open: true,
        message: "Gate pass information is updated successfully!",
        severity: "success",
      });
      resetForm();
      navigate("/storeMovent/create_gatepass_infomation"); //
    } catch (error) {
  ...
  return (
    <Box m="20px">
      <Header subtitle="Update good receiving note " />
      <Formik
        onSubmit={handleFormSubmit}
        initialValues={gatepassInformation}
...
export default UpdateGatePassInformation;
