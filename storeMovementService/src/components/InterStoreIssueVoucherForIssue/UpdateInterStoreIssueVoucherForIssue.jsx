...
import {
  ...
  getAllStores,
  getInterStoreIssueVoucherForIssuenById,
  updateInterStoreIssueVoucherForIssue,
} from "../../Api/storeMovementAp";
import { useLocation,useNavigate } from "react-router-dom";

const UpdateInterStoreIssueVoucherForIssue = () => {
  ...
  useEffect(() => {
    ...
    fetchInterStoreIssueVouncherById();
  }, []);
...
const handleFormSubmit = async (values, { resetForm }) => {
  ...
  try {
    await updateInterStoreIssueVoucherForIssue(tenantId,id, values);
    setNotification({
      open: true,
 ...
 const fetchAllStoreRequision = async () => {
   ...
   const fetchInterStoreIssueVouncherById = async () => {
              try {
                  const response = await getInterStoreIssueVoucherForIssuenById(tenantId,id);
                  setInterStore(response.data);
             
               } catch (error) {
                console.error("Error fetching inter store:", error.message);
      }
          };
 ...
 const [interStore, setInterStore] = useState({
  ...
  return (
    <Box m="20px">
      <Header subtitle="Update InterStore IssueVoucher For Issue" />
      <Formik
        onSubmit={handleFormSubmit}
        initialValues={interStore}
...
export default UpdateInterStoreIssueVoucherForIssue;
