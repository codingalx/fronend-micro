...
const UpdateCriterial = () => {
  ...
  const fetchAllCategory = async () => {
   ...
   const [getcriterials, setCriterials] = useState({
    name: "",
    weight: "",
    description: "",
    categoryId: "",
    status: "",
     ...
     const fetchCriterial = async () => {
        try {
          const response = await getCriterialById(tenantId, id);
          setCriterials(response.data);
          console.log(response.data);
        } catch (error) {
          console.error(error.message);
        }
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    try {
      const criterialNameExists = criterial.some(
        (criterial) => criterial.name === values.name && criterial.id !== id
      );
  ...
  const totalWeight = criterial.reduce((acc, curr) => {
    return acc + (curr.id !== id ? curr.weight : 0); // Exclude the current category's weight
      ...
      await updateCriterial(tenantId,id, values);
      setNotification({
        open: true,
  ...
  if (!id) {
    return <NotFoundHandle message="No criterial selected for updation." navigateTo="/evaluation/criterial" />;
  }
  ...
  return (
    <Box m="20px">
      <Header subtitle="Update criterial name " />
      <Formik
        onSubmit={handleFormSubmit}
        initialValues={getcriterials}
...
export default UpdateCriterial;
