...
const UpdateFixedAsset = () => {
  ...
  const [fixAsset, setFixedAsset] = useState({
    itemId: "",
    departmentId: "",
    farNo: "",
    deliveryStatus: "",
  ...
  const fetchFixedAssetById = async () => {
    try {
      const response = await getFixedAssetById(tenantId, id);
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      const response = await updatefixedAsset(tenantId, id, formValues);
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        setNotification({
          open: true,
          message: "Failed to updated fixed assets. Please try again.",
          severity: "error",
        });
      }
    } catch (error) {
      let errorMessage = "Failed to update fixed assets. Please try again.";
  ...
  return (
    <Box m="20px">
      <Header subtitle="Update fixed assets" />
      <Formik
        onSubmit={handleFormSubmit}
        initialValues={fixAsset}
