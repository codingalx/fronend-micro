...
import {
    ...
    getMortgageInfosById,
    listEmployee,
    updatepaMortgageInfos,
} from "../../../Api/payrollApi";
import { useLocation,useNavigate } from "react-router-dom";

const UpdateMortgageInfo = () => {
    ...
    const fetchMoratge = async () => {
        try {
            const response = await getMortgageInfosById(id);
            const data = response.data;
            setMortage(data);
        } catch (error) {
            console.error("Error fetching mortage:", error.message);
        }
    };
    ...
    if (bankInfoExists) {
      setNotification({
        open: true,
        message: "Bank information already exists. Please use a different one for updation",
        severity: "warning",
      });
      return;
    }

            await updatepaMortgageInfos(id,{ ...values, employeeId });
            setNotification({
                open: true,
                message: "Mortage information created successfully!",
                severity: "success",
            });
            resetForm();
            ...
            setNotification({
                open: true,
                message: "Failed to update mortage information. Please try again.",
                severity: "error",
            });
        }
    };
    ...
    return (
        <Box m="20px">
            <Header subtitle="Update Mortgage Information" />
            <Formik
                onSubmit={handleFormSubmit}
                initialValues={mortage}
...
export default UpdateMortgageInfo;
