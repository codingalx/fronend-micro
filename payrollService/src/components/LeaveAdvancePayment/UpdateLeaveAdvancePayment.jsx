...
import {  getpayLeaveAdvacementPaymentById, listEmployee, updatepaLeaveAdvacementPayment } from "../../../Api/payrollApi";
import { useLocation,useNavigate } from "react-router-dom";

const UpdateLeaveAdvancePayment = () => {
  ...
  const [employeeId, setEmployeeId] = useState("");
    const [leaveadvancePayement, setLeaveadvancePayement] = useState({
 startFrom: "",
    endTo: "",
    });
  ...
  const fetchAllEmployee = async () => {
    try {
      ...
      console.error("Error fetching employees:", error.message);
       ...
       const fetchLeaveAdvacePayment = async () => {
        try {
          const response = await getpayLeaveAdvacementPaymentById(id);
          const data = response.data;
          setLeaveadvancePayement(data);
        } catch (error) {
          console.error("Error fetching fetch single leave advance payment rate:", error.message);
        }
      };
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    try {
      await updatepaLeaveAdvacementPayment(id,{ ...values, employeeId });
      setNotification({
        open: true,
      ...
      setEmployeeId("");
        navigate('/payroll/create_leave_advance_payment');
    } catch (error) {
  ...
  return (
    <Box m="20px">
      <Header subtitle="Update Leave Advance Payment" />
      <Formik
        onSubmit={handleFormSubmit}
        initialValues={leaveadvancePayement}
...
export default UpdateLeaveAdvancePayment;