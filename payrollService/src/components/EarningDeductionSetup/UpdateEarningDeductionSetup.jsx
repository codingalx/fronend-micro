...
import {  getDeductionSetUpById, updatepeDeductionSetUp } from "../../../Api/payrollApi";
import { useLocation,useNavigate } from "react-router-dom";

const UpdateEarningDeductionSetup = () => {
  ...
  const [notification, setNotification] = useState({
    ...
    const [earningDeductionSetUp, setEarningDeductionSetUp] = useState({
  ...
  const handleCloseSnackbar = () => {
    ...
    useEffect(() => {
      fetchEarningDeduction();
    }, []);
  

   const fetchEarningDeduction = async () => {
      try {
        const response = await getDeductionSetUpById(id);
        const data = response.data;
        setEarningDeductionSetUp(data);
      } catch (error) {
        console.error("Error fetching fetch  earning deduction:", error.message);
      }
    };
...
const handleFormSubmit = async (values, { resetForm }) => {
  ...
  try {
    ...
    await updatepeDeductionSetUp(id,values);
    
    // Set success notification
    setNotification({
      open: true,
      message: "Earning deduction updated  successfully!",
      severity: "success",
    });
    
    // Reset the form after successful submission
    resetForm();
        navigate('/payroll/create_earning_eduction_setup');
    
 
  } catch (error) {
    // Log the error for debugging
    console.error("Failed to submit Earning deduction updated form data:", error);
    
    // Set error notification
    setNotification({
      open: true,
      message: "Failed to create Earning deduction updated. Please try again.",
      severity: "error",
    });
  }
};
  ...
  return (
    <Box m="20px">
      ...
      <Formik
        ...
        initialValues={earningDeductionSetUp}
...
export default UpdateEarningDeductionSetup;
