...
import { createPensionRates, getAllPensionRates } from "../../../Api/payrollApi";
import GetAllPensionRate from "./GetAllPensionRate";

const CreatePensionRate = () => {
  ...
  try {
      ...
      const pensionDateExists = pensionRate.some(
        (pension) => pension.date === values.date
    );
    ...
    await createPensionRates(values);
    setNotification({
      open: true,
      message: "pension rate created successfully!",
      severity: "success",
    });
    resetForm();
    ...
    setNotification({
      open: true,
      message: "Failed to create pension rate. Please try again.",
      severity: "error",
    });
  }
};
  ...
  return (
    <Box m="20px">
      ...
      <GetAllPensionRate refreshKey={refreshKey} />
    </Box>
  );
};

export default CreatePensionRate;
