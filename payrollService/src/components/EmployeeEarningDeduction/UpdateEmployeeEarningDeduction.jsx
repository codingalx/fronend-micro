...
const UpdateEmployeeEarningDeduction = () => {
...
const handleFormSubmit = async (values, { resetForm }) => {
  try {
    ...
    await updateEmployeeEarningDeduction(id,{  ...values, employeeId });
    ...
    navigate('/payroll/create_employee_earning_deduction'); 
  } catch (error) {
  ...
  const fetchAllEarningDeduction = async () => {
   ...
   const fetchEarningEmployeeDeduction = async () => {
    try {
      const response = await getEmployeeEarningDeductionsById(id);
      const data = response.data;

      setEmployeeEarningDeduction(data);

       if (data.employeeId) {
                 const employeeResponse = await getEmployeeById(tenantId, data.employeeId);
  ...
  return (
    <Box m="20px">
      <Header subtitle="Update Employee Earning Deduction" />
      <Formik
...
export default UpdateEmployeeEarningDeduction;
