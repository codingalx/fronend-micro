...
const UpdateFamilyDetail = () => {
  ...
  const { id } = location.state || {};
    ...
    const [erningEmployeeErningDeduction, setErningEmployeeErningDeduction] = useState([]);
        ...
        const [familyDetali, setFamilyDetali] = useState({
              firstName: "",
        middleName: "",
        lastName: "",
        bankName: "",
    ...
    const fetchAllEmployee = async () => {
        ...
        const fetchFamilyDetalil = async () => {
        try {
            const response = await getFamilyDetailsById(id);
            const data = response.data;
            setFamilyDetali(data);
        } catch (error) {
    ...
    const handleFormSubmit = async (values, { resetForm }) => {
        try {
            ...
            await updateFamilyDetails(id,{ ...values, employeeId });
       ...
       navigate('/payroll/create_family_detail'); 
            ...
            setNotification({
                open: true,
                message: "Failed to update family details. Please try again.",
                severity: "error",
            });
        }
    };
    ...
    const checkoutSchema = yup.object().shape({
        ...
        lastName: yup.string().required("lastName from is required"),

        appliedFrom: yup.string().required("applied from is required"),
        bankName: yup.string().required("bank Name is required"),
        firstName: yup.string().required("firts name is required"),
    ...
    return (
        <Box m="20px">
            <Header subtitle="Update Family Details" />
            <Formik
                onSubmit={handleFormSubmit}
                initialValues={familyDetali}
...
export default UpdateFamilyDetail;
