...
import { createPenalty, getAllPenalties } from "../../Api/disciplineApi";
...
const CreatePenalty = () => {
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    try {
      const penaltyExists = penalties.some(
        (penalty) => penalty.name === values.name
      );
      ...
      await createPenalty(tenantId, values);
      setNotification({
        open: true,
      ...
      setNotification({
        open: true,
        message: "Failed to create penalty. Please try again.",
        severity: "error",
      });
    }
  };
  ...
  const actionTakerOptions = [
    ...
    { value: " TOP_MANAGEMENT", label: "Top Managment" },
  ];
  ...
  return (
    <Box m="20px">
      <Header subtitle="Create Penalty" />
      <Formik
      ...
      <ListPenalty refreshKey={refreshKey} />
    </Box>
  );
};
