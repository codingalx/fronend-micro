...
import { getDisciplineUser, getEmployeeByEId, getOffense } from "../../Api/disciplineApi";
import { getEmployeeById } from "../../Api/disciplineApi";
import { useAtom } from 'jotai';
import { authAtom } from 'shell/authAtom';

const ListDisciplineUser = ({ refreshKey }) => {
  ...
  const [offenderId, setOffenderId] = useState(null);
  ...
  const fetchUserDisciplines = async (offenderId) => {
    try {
      const response = await getDisciplineUser(tenantId, offenderId);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setUserDisciplines(updatedDisciplines);
    } catch (error) {
      setError(error.message);
  ...
  const handleEditDiscipline = (id) => {
    navigate('/discipline/update-discipline', { state: { disciplineId: id } });
  };
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1 }}>
          {params.row.status === "PENDING" && (
            <>
              <Tooltip title="Update Discipline">
                <IconButton
                  onClick={() => handleEditDiscipline(params.row.id)}
                  color="primary"
                >
                  <EditIcon />
                </IconButton>
...
export default ListDisciplineUser;