...
const UpdateCriteria = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setNotification({
        open:true,
        message:"error fetching criteria to update",
        severity:"error"
      })
      setLoading(false);
    }
  ...
  const handleFormSubmit = async (values) => {
    try {
      ...
      await updatePromotionCriteriaName(tenantId, criteriaNameId, values);
      setNotification({
        open:true,
        message:"criteria updated successfully!",
        severity:"success"
      })
      setTimeout(() => navigate("/promotion/criteria_name"), 1500);
    } catch (error) {
