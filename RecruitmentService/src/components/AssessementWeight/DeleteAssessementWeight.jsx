...
import { deleteAssessment } from "../../../configuration/RecruitmentApp";
import { useAtom } from 'jotai';
import { authAtom } from 'shell/authAtom';



const DeleteAssessementWeight = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { weightId,recruitmentId} = location.state || {};
  ...
  const handleDelete = async () => {
    try {
      await deleteAssessment(tenantId,weightId);
      const id =recruitmentId ;
      navigate('/recruitment/more', { state: { id, activeTab: 2 } }); 
...
export default DeleteAssessementWeight;
