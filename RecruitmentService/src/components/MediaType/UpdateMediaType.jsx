...
import {  getMediaTypeById, listMediaType, updateMediaType } from "../../../configuration/RecruitmentApp";
...
const UpdateMediaType = () => {
  ...
  const fetchAllMediaTypes = async () => {
    try {
      ...
      const fetchMediaTypeById = async () => {
          try {
              const response = await getMediaTypeById(tenantId,id);
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    ...
    //     (mediaType) => mediaType.mediaTypeName === values.mediaTypeName && mediaType.id !== id
    //   );
    ...
    //   }
      ...
      await updateMediaType(tenantId,id, values);
      setNotification({
        open: true,
      ...
      navigate('/recruitment/media_type'); 
    } catch (error) {
  ...
  return (
    <Box m="20px">
      <Header subtitle="Update Media Type" />
      <Formik
        onSubmit={handleFormSubmit}
        initialValues={mediaNames}
...
export default UpdateMediaType;