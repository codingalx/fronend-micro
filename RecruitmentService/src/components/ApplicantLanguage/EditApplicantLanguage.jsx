...
import { editApplicantLanguages, getApplicantLanguagesbyId, listLanguageName } from "../../../configuration/RecruitmentApp";
import { useLocation } from 'react-router-dom';
import { useAtom } from 'jotai';
import { authAtom } from 'shell/authAtom';


const EditApplicantLanguage = () => {
  ...
  const handleFormSubmit = async (values) => {
    try {
      const response = await editApplicantLanguages(tenantId,applicantId, languageId, values);
  
      if (response.status === 200) {
        const id = applicantId;
        navigate("/recruitment/moreaboutapplicant", { state: { id } ,activeTab:2});
  ...
  const fetchLanguage = async () => {
    try {
      const response = await getApplicantLanguagesbyId(tenantId,applicantId,languageId);
...
export default EditApplicantLanguage;
