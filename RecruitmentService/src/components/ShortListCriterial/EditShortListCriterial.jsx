...
import { editCriteria, getCriteriaById, listEducationLevels  } from "../../../configuration/RecruitmentApp";
...
const EditShortListCriterial = () => {
  ...
  const fetchCriterials = async () => {
    try {
      const response = await getCriteriaById(tenantId,shortlistCriteriaId);
  ...
  const handleFormSubmit = async (values) => {
    try {
      ...
      await editCriteria(tenantId,shortlistCriteriaId, values);
      navigate('/recruitment/more', {
        state: { id: recruitmentId,  activeTab: 3 },
      });
    } catch (error) {
