...
import { approveTermination, getTermination } from "../../Api/separationApi";
...
const ApproveTermination = () => {
  ...
  if (!location.state) {
    return <NotPageHandle message="No Termination selected to Approve" navigateTo="/separation/list-termination" />;
  }

  const { tenantId, terminationId } = location.state;

  const fetchTerminationData = async () => {
    try {
      const response = await getTermination(tenantId, terminationId);
      const terminationData = response.data;
      setCurrentStatus(terminationData.status);
      setCurrentRemark(terminationData.remark || ""); 
    } catch (error) {
      console.error('Error fetching termination data:', error);
      setNotification({
        open: true,
        message: 'Failed to load termination data.',
        severity: 'error',
      });
    } finally {
      setLoading(false);
  ...
  const handleFormSubmit = async (values, { resetForm }) => {
    if (values.decision === currentStatus && values.remark === currentRemark) {
      setNotification({
        open: true,
        message: `Termination is already ${currentStatus.toLowerCase()} and remark unchanged. No changes made.`,
        severity: 'info',
      });
      return;
    }
    ...
    try {
      await approveTermination(tenantId, terminationId, values);
      setNotification({
        open: true,
        message: `Termination ${values.decision.toLowerCase()} successfully!`,
        severity: 'success',
      });
      navigate('/separation/list-termination');
    } catch (error) {
  ...
  return (
    <Box m="20px">
      <Header subtitle="Update Termination Status" />
